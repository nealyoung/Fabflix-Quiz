...
- (QuizQuestion *)getQuestionTypeNotDirectorForStar {    
    ...
    FMResultSet *correct = [database executeQuery:@"SELECT director FROM movies AS m WHERE NOT EXISTS (SELECT stars_in_movies.movie_id FROM stars_in_movies WHERE stars_in_movies.movie_id=m.id AND stars_in_movies.star_id=?) ORDER BY RANDOM() LIMIT 1;", [NSNumber numberWithInteger:starID]];
...
- (QuizItem *)randomMovie {
    FMResultSet *result = [database executeQuery:@"SELECT id, title, director FROM movies ORDER BY RANDOM() LIMIT 1;"];
...
- (QuizItem *)randomMovieWithMinimumStars:(NSInteger)stars {
    FMResultSet *result = [database executeQuery:@"SELECT id, title, director FROM movies WHERE (SELECT count(stars_in_movies.star_id) FROM stars_in_movies WHERE stars_in_movies.movie_id=movies.id) >= ? ORDER BY RANDOM() LIMIT 1;", [NSNumber numberWithInteger:stars]];
